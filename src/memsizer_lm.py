...
from .memsizer import Embedding, MemsizerDecoder

DEFAULT_MAX_TARGET_POSITIONS = 1024


@register_model('memsizer_lm')
class MemsizerLanguageModel(FairseqLanguageModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--use-memsizer', action='store_true',
                            help='use rfa')
    ...
    def build_model(cls, args, task):
        ...
        decoder = MemsizerDecoder(
                args, task.target_dictionary, embed_tokens, no_encoder_attn=True, use_memsizer=args.use_memsizer
            )
        return cls(decoder)
...
@register_model_architecture('memsizer_lm', 'memsizer_lm')
def base_lm_architecture(args):
    ...
    args.use_memsizer = getattr(args, "use_memsizer", True)
    args.causal_proj_dim = getattr(args, "causal_proj_dim", 32)
    args.q_init_scale = getattr(args, "q_init_scale", 32.)
    args.kv_init_scale = getattr(args, "kv_init_scale", 1.)

@register_model_architecture('memsizer_lm', 'memsizer_lm_big')
def memsizer_lm_big(args):
...
@register_model_architecture('memsizer_lm', 'memsizer_lm_wiki103')
@register_model_architecture('memsizer_lm', 'memsizer_lm_baevski_wiki103')
def memsizer_lm_baevski_wiki103(args):
    ...
    memsizer_lm_big(args)


@register_model_architecture('memsizer_lm', 'memsizer_lm_gbw')
@register_model_architecture('memsizer_lm', 'memsizer_lm_baevski_gbw')
def memsizer_lm_baevski_gbw(args):
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
    args.dropout = getattr(args, 'dropout', 0.1)
    args.attention_dropout = getattr(args, 'attention_dropout', 0.1)
    args.no_decoder_final_norm = getattr(args, 'no_decoder_final_norm', True)
...
@register_model_architecture('memsizer_lm', 'memsizer_lm_gpt')
def memsizer_lm_gpt(args):
...
@register_model_architecture('memsizer_lm', 'memsizer_lm_gpt2_small')
def memsizer_lm_gpt2_small(args):
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 1024)
    args.decoder_ffn_embed_dim = getattr(args, 'decoder_ffn_embed_dim', 4096)
...
@register_model_architecture('memsizer_lm', 'memsizer_lm_gpt2_medium')
def memsizer_lm_gpt2_medium(args):
...
@register_model_architecture('memsizer_lm', 'memsizer_lm_gpt2_big')
def memsizer_lm_gpt2_big(args):
